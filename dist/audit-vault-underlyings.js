...
import { ERC20_PROBE_ABI, classifyTokenProbe, probeMulticall, unwrapProbe, } from "./fetch/morpho/stubUnderlying.js";
...
async function codeSizes(chainId, addresses) {
    ...
    for (const address of addresses) {
        ...
        for (const client of clients) {
            try {
                const code = (await client.getCode({ address: address })) ?? "0x";
...
async function main() {
    ...
    const totalVaults = [...byChain.values()].reduce((a, m) => a + [...m.values()].reduce((b, l) => b + l.length, 0), 0);
    console.error(`Probing ${totalTokens} unique underlyings behind ${totalVaults} vaults on ${byChain.size} chains`);
    const findings = [];
    const unreachable = [];
    await Promise.all([...byChain.entries()].map(async ([chainId, tokens]) => {
        ...
        const [meta, sizes, vmeta] = await Promise.all([
            multicall(chainId, addrs.flatMap((address) => ["name", "symbol", "decimals", "totalSupply"].map((name) => ({
                address,
                name,
                args: [],
            }))), ERC20_PROBE_ABI),
            codeSizes(chainId, addrs),
            multicall(chainId, vaults.flatMap(({ vault }) => ["name", "symbol", "totalAssets"].map((name) => ({
                address: vault,
                name,
                args: [],
            }))), VAULT_ABI),
    ...
    const order = { "no-code": 0, stub: 1, suspicious: 2, unreachable: 3, ok: 4 };
    ...
    for (const f of flagged) {
        const vs = f.vaults
            ...
            .join("<br>");
        lines.push(`| ${f.chainId} | ${f.underlying} | ${f.verdict} | ${f.token.codeBytes ?? "?"}B | ${f.token.name ?? "∅"} / ${f.token.symbol ?? "∅"} / ${f.token.decimals ?? "∅"} | ${vs} | ${f.reasons.join("; ")} |`);
    }
