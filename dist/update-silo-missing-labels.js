...
async function fetchUnnamed(chains, maxRiskScore) {
    ...
    for (const it of items) {
        ...
        if (!/^SILO_V[23]_/.test(key) || names[key])
            continue;
...
async function main() {
    ...
    console.log(`Found ${total} unnamed Silo markets across ${Object.keys(byChain).length} chain(s)`);
    ...
    for (const [chainId, keys] of Object.entries(byChain)) {
        const configs = keys.map((key) => ({
            key,
            version: key.slice("SILO_V".length, "SILO_V".length + 1),
    ...
    if (skipped.length) {
        console.warn(`Skipped ${skipped.length} (no on-chain silos/symbols): ${skipped.map((k) => k.slice(0, 20)).join(", ")}`);
    }
    process.exit(0);
}
main().catch((err) => {
