...
async function main() {
    const vaultsData = readJsonFile(VAULTS_FILE);
    ...
    for (const chainId of FEATHER_CHAIN_IDS) {
        ...
        const existing = new Set((markets[FORK][chainId] ?? []).map((s) => s.toLowerCase()));
        const before = existing.size;
        for (const id of ids)
            existing.add(id);
        added += existing.size - before;
        markets[FORK][chainId] = [...existing].sort((a, b) => a.localeCompare(b));
        console.log(`chain ${chainId}: ${vaults.length} vaults -> ${ids.length} market ids (${existing.size} total)`);
    }
