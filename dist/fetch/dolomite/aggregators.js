...
const DEPLOYMENTS_URL = "https://raw.githubusercontent.com/dolomite-exchange/dolomite-margin-modules/master/packages/deployment/src/deploy/deployments.json";
// DolomiteAggregator name → candidate contract keys, newest version first.
const AGGREGATOR_CONTRACTS = {
 ...
 */
export async function fetchDolomiteAggregatorTraders(chainIds) {
    let deployments;
    try {
    ...
    for (const chainId of chainIds) {
        const entry = {};
        for (const [aggregator, candidates] of Object.entries(AGGREGATOR_CONTRACTS)) {
