...
import { dolomiteRiskOverrideSetterAbi, DOLOMITE_CATEGORIES, DOLOMITE_RISK_FEATURES, DOLOMITE_SUBGRAPH_URLS, DOLOMITE_FALLBACK_RPCS, } from "./constants.js";
const ZERO = "0x0000000000000000000000000000000000000000";
const dec = (v) => Number(BigInt(v?.value ?? v ?? 0)) / 1e18;
/**
 ...
 */
export async function getDolomiteSetter(chainId) {
    ...
    try {
        ...
        const setter = json?.data?.dolomiteMargins?.[0]?.defaultAccountRiskOverrideSetter;
        if (!setter || setter.toLowerCase() === ZERO)
            return null;
...
async function readSetter(chainId, setter, calls) {
    ...
    try {
        ...
        return Promise.all(full.map((c) => client
            .readContract({
            address: setter,
            abi: dolomiteRiskOverrideSetterAbi,
            functionName: c.name,
 ...
 */
export async function fetchDolomiteEmode(chainId, setter, marketIds) {
    // 1. category params for BERA/BTC/ETH/STABLE (skip NONE=0)
    const catParams = await readSetter(chainId, setter, [1, 2, 3, 4].map((c) => ({ name: "getCategoryParamByCategory", args: [c] })));
    const categories = {};
    ...
    const catByMarket = await readSetter(chainId, setter, marketIds.map((m) => ({ name: "getCategoryByMarketId", args: [BigInt(m)] })));
    const featByMarket = await readSetter(chainId, setter, marketIds.map((m) => ({ name: "getRiskFeatureByMarketId", args: [BigInt(m)] })));
    const marketCategories = {};
    const riskFeatures = {};
    const singleCollateralMarkets = [];
    marketIds.forEach((m, i) => {
        const cat = Number(catByMarket[i] ?? 0);
        if (cat > 0)
            marketCategories[m] = DOLOMITE_CATEGORIES[cat];
    ...
    if (singleCollateralMarkets.length > 0) {
        const scParams = await readSetter(chainId, setter, singleCollateralMarkets.map((m) => ({
        ...
        singleCollateralMarkets.forEach((m, i) => {
            ...
            if (Array.isArray(arr)) {
                riskFeatures[m].singleCollateral = arr.map((s) => ({
                    debtMarketIds: (s.debtMarketIds ?? []).map((d) => d.toString()),
