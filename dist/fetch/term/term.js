...
const SUBGRAPH_BY_CHAIN = {
    ...
    "43114": "https://api.subgraph.ormilabs.com/api/public/05e9a4e2-103b-4163-a81e-3b1b038d0055/subgraphs/term-finance-avalanche/latest/gn",
    "56": "https://api.subgraph.ormilabs.com/api/public/05e9a4e2-103b-4163-a81e-3b1b038d0055/subgraphs/term-finance-bnb/latest/gn",
    "8453": "https://api.subgraph.ormilabs.com/api/public/05e9a4e2-103b-4163-a81e-3b1b038d0055/subgraphs/term-finance-base/latest/gn",
    "42161": "https://api.subgraph.ormilabs.com/api/public/05e9a4e2-103b-4163-a81e-3b1b038d0055/subgraphs/term-finance-arbitrum/latest/gn",
};
/**
...
async function gql(url, query, variables) {
    try {
        ...
        console.log("Term: subgraph fetch failed:", e?.message ?? e);
        return null;
    }
}
const REPOS_QUERY = `
...
async function fetchAllPages(url, query, variables, field) {
    const out = [];
 ...
 */
export function pickAuctionForRepo(auctions, nowSec) {
...
function toMarket(repo, auction) {
    if (!repo?.id || !repo.purchaseToken)
        return null;
    const metaOf = (addr) => (repo.collateralTokensMeta ?? []).find((m) => String(m.id).toLowerCase() === addr.toLowerCase());
    const damagesOf = (addr) => (repo.liquidatedDamagesSchedule ?? []).find((d) => String(d.collateralToken).toLowerCase() === addr.toLowerCase())?.liquidatedDamages;
    ...
    const collSyms = collateralParams
        .map((c) => metaOf(c.token)?.symbol ?? "?")
        .join(" / ") || "?";
    return {
        ...
        collateralManager: String(repo.termRepoCollateralManager ?? "").toLowerCase(),
        ...
        redemptionValue: String(repo.termRepoTokenRedemptionRatio ?? "1000000000000000000"),
 ...
 */
export class TermMarketsUpdater {
    ...
    async fetchData() {
        ...
        for (const chainId of chainIds) {
            ...
            const repos = await fetchAllPages(url, REPOS_QUERY, { cutoff: String(cutoff) }, "termRepos");
            if (repos.length === 0) {
                console.log(`Term: chain ${chainId}: 0 live repos`);
                result[chainId] = [];
                continue;
            }
            const auctions = await fetchAllPages(url, AUCTIONS_QUERY, { terms: repos.map((r) => r.id.toLowerCase()) }, "termAuctions");
            ...
            const markets = repos
                .map((r) => toMarket(r, pickAuctionForRepo(byTerm.get(r.id.toLowerCase()) ?? [], nowSec)))
                .filter((m) => m !== null)
                .sort((a, b) => String(a.termRepoId) < String(b.termRepoId) ? -1 : 1);
            const openNow = markets.filter((m) => Number(m.maturity) > nowSec).length;
            console.log(`Term: chain ${chainId}: ${markets.length} repos (${openNow} unmatured)`);
        ...
        return {
            ...
            [LABELS_FILE]: buildTermLabels(result),
        };
    }
    /**
...
export const termLenderKey = (repoId) => `TERM_FINANCE_${repoId.replace(/^0x/i, "").toUpperCase()}`;
/**
 ...
 */
export function buildTermLabels(markets) {
    // Base lender label (the bare key the user-data path uses).
    const names = { TERM_FINANCE: "Term Finance" };
    const shortNames = { TERM_FINANCE: "Term" };
