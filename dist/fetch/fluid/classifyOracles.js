...
function providerFromContractName(name) {
    if (!name)
        return null;
    const n = name.toLowerCase();
    if (/wsteth|weeth|ezeth|rseth|wsteth|msteth|susde|susds|sdai|sweth|cdceth|lst|exchangerate|erc4626/.test(n))
        return "exchange-rate";
    if (/dexsmart|\bdex\b|peg/.test(n))
        return "composite"; // Fluid DEX / peg composite
    if (/univ3/.test(n) && !/clrs|check/.test(n))
        return "uniswap"; // primary TWAP
    if (/clrs|chainlink|fallback/.test(n))
        return "chainlink";
    if (/redstone/.test(n))
        return "redstone";
    return null;
}
const sourcifyNameCache = new Map();
/** Fetch the verified contract name for an oracle from Sourcify (keyless). */
async function fetchSourcifyName(chainId, addr) {
    const key = `${chainId}:${addr.toLowerCase()}`;
    if (sourcifyNameCache.has(key))
        return sourcifyNameCache.get(key);
    let name = null;
    try {
        const ctrl = new AbortController();
        const t = setTimeout(() => ctrl.abort(), 12000);
        const res = await fetch(`https://sourcify.dev/server/v2/contract/${chainId}/${addr}?fields=compilation`, { signal: ctrl.signal });
        clearTimeout(t);
        if (res.ok) {
...
async function resolveFluidProviders(chainId, oracles) {
    const out = new Map();
    const CONC = 8;
...
export async function classifyFluidOracles() {
    const fluidVaults = readJsonFile(fluidVaultsFile);
    const result = {};
    for (const [chainId, byVault] of Object.entries(fluidVaults)) {
        ...
        for (let off = 0; off < vaults.length; off += CHUNK) {
            ...
            console.log(`  Fluid [${chainId}]: vault oracles ${Math.min(off + CHUNK, vaults.length)}/${vaults.length}`);
        }
        ...
        const oracles = [...new Set([...oracleByVault.values()].filter((o) => !!o))];
        ...
        const symOf = (a) => (a ? symbols.get(a) ?? null : null);
        result[chainId] = {};
        for (const [vaultRaw, raw] of Object.entries(byVault)) {
            ...
            const provider = (oracle ? providerByOracle.get(oracle) : null) ??
