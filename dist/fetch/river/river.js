...
const TM_ABI = TM_READS.map((name) => ({
    ...
    outputs: [
        {
            name: "",
            type: name === "paused" || name === "sunsetting"
                ? "bool"
...
const num = (v) => typeof v === "bigint" || typeof v === "number" ? String(v) : undefined;
...
async function fetchChain(lender, chainId, cfg) {
    ...
    const tmAddrs = (await multicallRetryUniversal({
        chain: chainId,
    ...
    const tmRes = (await multicallRetryUniversal({
        chain: chainId,
        calls: tmAddrs.flatMap((tm) => TM_READS.map((name) => ({ address: tm, name, args: [] }))),
        abi: TM_ABI,
        allowFailure: true,
    }));
            ...
            for (const [chainId, cfg] of Object.entries(config[lender])) {
                try {
                    ...
                    for (const m of data.markets) {
                        const coll = m.name?.split(" / ").pop() ?? `#${m.index}`;
                    ...
                    console.log(`River: ${lender} chain ${chainId} failed:`, e?.shortMessage ?? e?.message ?? e);
                }
            }
        }
