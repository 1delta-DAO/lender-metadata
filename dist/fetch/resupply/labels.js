...
export const RESUPPLY_REGISTRY = "0x10101010E0C3171D894B71B3400668aF311e7D94";
const ETHEREUM_RPCS = [
    "https://eth.merkle.io",
 ...
 */
export function resupplyLabelFromName(rawName) {
...
export const resupplyLenderKey = (pair) => `RESUPPLY_${RESUPPLY_CHAIN_ID}_${pair.replace(/^0x/i, "").toUpperCase()}`;
/**
 ...
 */
export async function discoverResupplyPairs() {
    const pc = client();
    ...
    for (const pair of addresses ?? []) {
        try {
            ...
            console.log(`Resupply: ${pair} name() failed:`, e?.shortMessage ?? e?.message ?? e);
        }
    }
    return out;
}
export function buildResupplyLabels(pairs) {
    const names = { RESUPPLY: "Resupply" };
    const shortNames = { RESUPPLY: "Resupply" };
