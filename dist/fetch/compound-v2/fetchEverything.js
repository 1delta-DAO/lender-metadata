...
export async function fetchCompoundV2TypeTokenData() {
    ...
    for (const chain of Object.keys(chainToForks)) {
        ...
        for (let i = 0; i < forksOnChain.length; i++) {
            ...
            const oracle = typeof oracleResult === "string" && isAddress(oracleResult)
                ? oracleResult
                : undefined;
            if (!oracle) {
                console.warn(`  ${fork} on ${chain}: oracle() read failed (${JSON.stringify(oracleResult)}) — not publishing an oracle for this fork`);
            }
            forkMarketData.push({
                fork,
