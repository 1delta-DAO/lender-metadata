...
import { encodeFunctionData, HttpRequestError, TimeoutError, } from "viem";
/**
 ...
 */
async function probeOne(chain, cToken) {
    let sawRevert = false;
 ...
 */
export async function findNativeMarkets(chain, cTokens) {
    const out = new Set();
    for (const cToken of cTokens) {
        const isNative = await probeOne(chain, cToken);
        if (isNative)
            out.add(cToken.toLowerCase());
    }
    return out;
}
