...
let hollowRejects = 0;
const isKnown = (v) => v !== null && v !== undefined && v !== "" && v !== "UNKNOWN";
/**
 ...
 * for is preserved.
 */
function isHollowerThan(next, prev) {
    if (!prev || !next)
        return false;
    let lost = 0;
    let gained = 0;
...
export class MorphoOracleDataUpdater {
    ...
    mergeData(oldData, data, _fileKey) {
        ...
        if (hollowRejects > 0) {
            console.warn(`[morpho-oracles-data] kept ${hollowRejects} existing entries whose refresh came back hollower — the run could not read what the file already knows (rate limiting is the usual cause)`);
        }
