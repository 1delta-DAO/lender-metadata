...
async function fetchOracleConfigs(chainId, oracles) {
    ...
    if (unreachable.length > 0) {
        console.warn(`[morpho-oracles-data] chain ${chainId}: ${unreachable.length}/${oracles.length} oracles unreachable this run — their markets are OMITTED so the existing entries survive the merge`);
    }
...
export async function fetchMorphoOracleData(injectedByChain = {}, onlyInjected = false) {
    ...
    for (const chainId of chainIds) {
        ...
        const { configs: oracleConfigs, isV2Map, readOkMap, } = await fetchOracleConfigs(chainId, oracles);
        ...
        if (noSignalOracles.length > 0) {
            ...
            if (newUnderlyings.length > 0) {
                ...
                const { configs: underlyingConfigs, isV2Map: underlyingIsV2Map, readOkMap: underlyingReadOkMap, } = await fetchOracleConfigs(chainId, newUnderlyings);
        ...
        const marketTokenSymbols = {};
        if (newMarketTokenAddrs.length > 0) {
            console.log(`Morpho oracles [${chainId}]: fetching ${newMarketTokenAddrs.length} market token symbols`);
