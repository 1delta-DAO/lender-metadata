...
const MAX_PAGES = 50;
export function hasExplorerLogApi(chainId) {
 ...
 */
export async function fetchLogsFromExplorer(chainId, address, topic0) {
    const base = EXPLORER_LOG_API[chainId];
    if (!base)
        throw new Error(`no explorer log API for chain ${chainId}`);
    const out = [];
    const seen = new Set();
    let fromBlock = 0;
    for (let page = 0; page < MAX_PAGES; page++) {
        ...
        const body = await withRetry(async () => {
            ...
            if (!res.ok) {
                throw new Error(`explorer ${base} chain ${chainId}: ${res.status} ${res.statusText}`);
            }
        ...
        if (!Array.isArray(body.result)) {
            if (body.status === "0")
                break;
            throw new Error(`explorer ${base} chain ${chainId}: unexpected body ${JSON.stringify(body).slice(0, 200)}`);
        }
        const batch = body.result;
        let maxBlock = fromBlock;
        for (const log of batch) {
            const block = parseInt(log.blockNumber, 16);
            if (Number.isFinite(block) && block > maxBlock)
                maxBlock = block;
