...
import { computeMorphoMarketId, lenderKeyToMorphoMarketId, marketTripletKey, morphoMarketIdToLenderKey, } from "./morphoMarketId.js";
describe("computeMorphoMarketId", () => {
    it("matches Morpho docs example (mainnet USDT / Pendle market)", () => {
        ...
        expect(id).toBe("0x9a33209eee9e93f5f7aed04085f9f5e0ce9a7a103c476f5c30a0e5ca03c3d540");
    });
    it("marketTripletKey is stable", () => {
        expect(marketTripletKey("0xA", "0xb", "0xc")).toBe("0xa:0xb:0xc");
    });
    it("morphoMarketIdToLenderKey round-trips with lenderKeyToMorphoMarketId", () => {
        const id = "0x9a33209eee9e93f5f7aed04085f9f5e0ce9a7a103c476f5c30a0e5ca03c3d540";
...
describe("lender-labels MORPHO_BLUE_* suffix (optional consistency check)", () => {
    it("suffix matches marketId hex without 0x uppercase for sampled keys", () => {
        const raw = readFileSync(new URL("../../../data/lender-labels.json", import.meta.url), "utf8");
        const labels = JSON.parse(raw);
        const names = labels.names ?? {};
        const morphoKeys = Object.keys(names).filter((k) => k.startsWith("MORPHO_BLUE_") && k !== "MORPHO_BLUE");
