...
export const tellerLenderKey = (pool) => `TELLER_${pool.replace(/^0x/i, "").toUpperCase()}`;
/**
 ...
 */
export function buildTellerLabels(pools) {
    const names = { TELLER: "Teller" };
    const shortNames = { TELLER: "Teller" };
 ...
 */
export class TellerPoolsUpdater {
    name = "Teller Pools";
    async fetchData() {
        const data = await fetchTellerPoolsOnChain();
        return { [poolsFile]: data, [labelsFile]: buildTellerLabels(data) };
    }
    /**
