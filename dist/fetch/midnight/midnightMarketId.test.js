...
const deriveId = (chainId, midnight, m) => MarketUtils.toId({
...
describe("midnight-markets.json self-consistency", () => {
    ...
    it("every stored market re-derives its own id", () => {
        ...
        for (const [chainId, list] of Object.entries(markets)) {
            ...
            expect(midnight, `no core address configured for chain ${chainId}`).toBeTruthy();
            for (const m of list) {
                expect(deriveId(chainId, midnight, m).toLowerCase(), `market ${m.marketId} (${m.name ?? "?"}) does not hash to its own struct`).toBe(m.marketId.toLowerCase());
                checked++;
            }
        }
        expect(checked, "no markets to check — the roster is empty").toBeGreaterThan(0);
    });
    /**
    ...
    it("does not normalise collateral order (mixed leg orders survive)", () => {
        const list = (markets["8453"] ?? []).filter((m) => m.collateralParams.length > 1);
        if (list.length === 0)
            return; // no multi-leg markets in the roster right now
        ...
        for (const m of list) {
            expect(deriveId("8453", config["8453"].midnight, m).toLowerCase()).toBe(m.marketId.toLowerCase());
        }
    });
    /**
    ...
    it("labels are collision-free across every Midnight market", () => {
        ...
        const keys = new Set(Object.values(markets)
            .flat()
            .map((m) => `MORPHO_MIDNIGHT_${m.marketId.slice(2).toUpperCase()}`));
        for (const field of ["names", "shortNames"]) {
            const mine = Object.entries(labels[field] ?? {}).filter(([k]) => keys.has(k));
            expect(mine.length, `no ${field} written for Midnight markets`).toBe(keys.size);
            const seen = new Map();
            for (const [k, v] of mine) {
                expect(seen.has(v), `${field} collision on "${v}" (${seen.get(v)} vs ${k})`).toBe(false);
                seen.set(v, k);
            }
        }
    });
...
describe("classifyGating", () => {
    ...
    it("never reports an unrecognised gate as open", () => {
        const g = classifyGating("0x18a0E42C421dF95b895EF909756F56C662a9bdF7", ZERO);
...
describe("curate", () => {
    const mk = (maturity, totalUnits) => ({ marketId: "0x1", maturity: String(maturity), totalUnits });
...
describe("selectCarryForward", () => {
    const NOW = 1_800_000_000;
    const mk = (maturity) => ({ marketId: "0xAB", maturity: String(maturity) });
    ...
    it("retires a market only when matured AND proven empty on-chain", () => {
        const { carried, retired } = selectCarryForward([mk(NOW - 1)], { "0xab": 0n }, NOW);
