...
async function fetchAllBooks(apiBase) {
    const base = apiBase.replace(/\/+$/, "");
    const out = [];
    let cursor = null;
    for (let i = 0; i < 200; i++) {
        const url = `${base}/books?limit=20${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`;
        ...
        for (const chainId of chainIds) {
            ...
            if (!books) {
                try {
                    ...
                    console.log(`Midnight: book fetch failed (${apiBase}):`, e?.message ?? e);
                    books = [];
                }
                byApi.set(apiBase, books);
            }
            const chainBooks = books.filter((b) => String(b.chain_id) === String(chainId));
            ...
            const fees = midnight
                ? await resolveMarketFees(chainId, midnight, chainBooks.map((b) => b.market_id))
                : {};
            ...
            const markets = chainBooks
                ...
                const collSym = (b.collaterals ?? []).map((c) => sym(c.token)).join("/") || "?";
                ...
                const entry = {
                    ...
                    collateralParams: (b.collaterals ?? []).map((c) => ({
                        token: c.token,
                        lltv: String(c.lltv),
