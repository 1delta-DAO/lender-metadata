...
import { fetchTenorFixedMarkets } from "./tenorApi.js";
import { fetchMorphoMidnightMarkets, } from "./morphoApi.js";
...
const midnightEnumKey = (marketId) => `MORPHO_MIDNIGHT_${marketId.slice(2).toUpperCase()}`;
const isoDate = (unixSecs) => new Date(unixSecs * 1000).toISOString().slice(0, 10);
const ZERO = "0x0000000000000000000000000000000000000000";
const isZeroAddr = (a) => !a || a.toLowerCase() === ZERO;
 ...
 */
async function fetchRoster(chainId, cfg) {
    ...
    try {
        ...
        if (forChain.length > 0)
            ...
            console.log(`Midnight: Tenor returned no markets for chain ${chainId} — falling back to Morpho`);
    }
    catch (e) {
        console.log(`Midnight: Tenor roster failed (${e?.message ?? e}) — falling back to Morpho`);
    }
    if (tenor) {
        ...
        try {
            ...
            console.log(`Midnight: Morpho enrichment unavailable (${e?.message ?? e}) — marketFamilyId/listed omitted`);
        }
        const markets = tenor.map((m) => {
 ...
 */
function verifyMarketIds(chainId, midnight, markets) {
    const kept = [];
    for (const m of markets) {
        ...
        try {
            ...
            console.log(`Midnight: market ${m.marketId} id derivation threw (${e?.message ?? e}) — DROPPED`);
            continue;
        }
        if (derived.toLowerCase() !== m.marketId.toLowerCase()) {
            console.log(`Midnight: market ${m.marketId} struct hashes to ${derived} — DROPPED (source served a bad struct)`);
            continue;
        }
        kept.push(m);
    }
 ...
 */
export function curate(markets, nowSec) {
    const kept = markets.filter((m) => {
        if (Number(m.maturity) > nowSec)
            return true;
 ...
 */
export function selectCarryForward(missing, unitsById, nowSec) {
    const carried = [];
    const retired = [];
 ...
 */
function rosterFromStoredEntry(chainId, e, totalUnits) {
    const vaultLeg = (e.collateralParams ?? []).find((c) => c.collateralVault);
    return {
        marketId: e.marketId,
        chainId,
        loanToken: e.loanToken,
        collaterals: (e.collateralParams ?? []).map((c) => ({
            token: c.token,
            oracle: c.oracle,
            lltv: String(c.lltv),
 ...
 * as zero.
 */
async function resolveTotalUnits(chainId, midnight, marketIds) {
    const out = {};
    ...
    try {
        ...
        console.log(`Midnight: totalUnits multicall failed on chain ${chainId}:`, e?.shortMessage ?? e?.message ?? e);
        return out;
    }
...
async function resolveTokenMeta(chainId, tokens) {
    ...
    try {
        ...
        console.log(`Midnight: decimals multicall failed on chain ${chainId}:`, e?.shortMessage ?? e?.message ?? e);
    }
    tokens.forEach((t, i) => {
        ...
        const decimals = typeof dRaw === "bigint" || typeof dRaw === "number"
            ? Number(dRaw)
            : (known?.decimals ?? 18);
        const symbol = typeof sRaw === "string" && sRaw.length > 0 ? sRaw : known?.symbol;
 ...
 */
async function resolveMarketFees(chainId, midnight, marketIds) {
    const out = {};
    if (marketIds.length === 0)
        return out;
    ...
    try {
        ...
        console.log(`Midnight: marketState multicall failed on chain ${chainId}:`, e?.shortMessage ?? e?.message ?? e);
        return out;
    }
    marketIds.forEach((id, i) => {
        const r = res[i];
 ...
 */
export function classifyGating(enterGate, liquidatorGate) {
    return {
        enter: isZeroAddr(enterGate) ? "open" : "unknown",
 ...
 */
function legLabel(token, loanSymbol, collateralVault, sym) {
    if (collateralVault &&
...
export class MidnightUpdater {
    ...
    async fetchData() {
        ...
        for (const chainId of chainIds) {
            ...
            if (!midnight) {
                console.log(`Midnight: chain ${chainId} has no core address in config — skipping`);
                result[chainId] = [];
                continue;
            }
            let roster;
            ...
            try {
                ...
                console.log(`Midnight: chain ${chainId}: roster unavailable from both sources (${e?.message ?? e}) — keeping previous data`);
                result[chainId] = [];
                continue;
            }
            const verified = verifyMarketIds(chainId, midnight, roster);
            const { kept, dropped } = curate(verified, nowSec);
            console.log(`Midnight: chain ${chainId}: ${kept.length} markets from ${source} ` +
            ...
            const prevChain = Array.isArray(previous[chainId])
                ? previous[chainId]
                : [];
            const verifiedIds = new Set(verified.map((m) => m.marketId.toLowerCase()));
            const missing = prevChain.filter((e) => e?.marketId && !verifiedIds.has(e.marketId.toLowerCase()));
            if (missing.length > 0) {
                const units = await resolveTotalUnits(chainId, midnight, missing.map((e) => e.marketId));
                const { carried, retired } = selectCarryForward(missing, units, nowSec);
                for (const e of retired)
                    console.log(`Midnight: chain ${chainId}: market ${e.marketId} (${e.name ?? "?"}) left the ${source} roster, matured and empty on-chain — retired`);
                if (carried.length > 0) {
                    const rosters = carried.map((e) => rosterFromStoredEntry(chainId, e, units[e.marketId.toLowerCase()]));
                    const ok = verifyMarketIds(chainId, midnight, rosters);
                    for (const m of ok)
                        console.log(`Midnight: chain ${chainId}: market ${m.marketId} missing from the ${source} roster but ` +
            ...
            for (const m of kept) {
                ...
                for (const c of m.collaterals)
                    tokens.add(c.token.toLowerCase());
            }
            const meta = await resolveTokenMeta(chainId, [...tokens]);
            const fees = await resolveMarketFees(chainId, midnight, kept.map((m) => m.marketId));
            const dec = (a) => meta[a.toLowerCase()]?.decimals ??
                KNOWN_META[a.toLowerCase()]?.decimals ??
                18;
            const sym = (a) => meta[a.toLowerCase()]?.symbol ??
            ...
            const markets = kept
                ...
                const legSyms = m.collaterals.map((c) => legLabel(c.token, loanSym, m.collateralVault, sym));
                ...
                if (f) {
                    ...
                    if (m.settlementFeeCbp)
                        entry.settlementFeeCbp = m.settlementFeeCbp;
                    if (m.continuousFee !== undefined)
                        entry.continuousFee = m.continuousFee;
                }
                if (m.marketFamilyId)
                    entry.marketFamilyId = m.marketFamilyId;
                if (m.seriesId)
                    entry.tenorSeriesId = m.seriesId;
                if (typeof m.listed === "boolean")
                    entry.listed = m.listed;
                if (m.deprecatedAt)
                    entry.deprecatedAt = m.deprecatedAt;
    ...
    mergeData(oldData, data, fileKey) {
        ...
        for (const [chainId, markets] of Object.entries((data ?? {}))) {
