...
const isAddr = (a) => typeof a === "string" && /^0x[0-9a-f]{40}$/i.test(a);
/**
 ...
 */
export async function fetchMidnightOracleData() {
    const raw = readJsonFile(midnightMarketsFile);
    const injectedByChain = {};
    for (const [chainId, markets] of Object.entries(raw ?? {})) {
        if (chainId.startsWith("_") || !Array.isArray(markets))
            continue;
        const rows = [];
        for (const m of markets) {
            if (!m?.marketId || !isAddr(m.loanToken))
                continue;
            for (const cp of m.collateralParams ?? []) {
                if (!isAddr(cp?.oracle) || !isAddr(cp?.token))
                    continue;
