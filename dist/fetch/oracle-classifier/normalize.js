 ...
 */
export function decodeBytes32String(raw) {
    let hex;
 ...
 */
export function normalizeRedStoneDescription(desc) {
 ...
 */
export function normalizeGenericDescription(desc) {
...
export function parsePair(desc) {
    if (!desc)
        return null;
...
export function symbolsMatch(a, b) {
    ...
    const stripW = (s) => (s.startsWith("W") && s.length >= 4 ? s.slice(1) : s);
 ...
 * nothing cancels.
 */
export function synthesizeFromPairs(pairs) {
    const valid = pairs.filter((p) => p !== null);
    if (valid.length === 0)
        return null;
    if (valid.length === 1)
        return `${valid[0].base} / ${valid[0].quote}`;
