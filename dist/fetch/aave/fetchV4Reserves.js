...
function isEmptyOrZeroAddr(s) {
    return !s || s === ZERO_ADDR;
}
function needsGetReserveRetry(entry) {
...
function mergeGetReserveResult(entry, result) {
    if (result == null)
        return;
    ...
    if (h)
        entry.hub = h;
    if (u || h) {
        entry.assetId = Number((isArray ? result[2] : result?.assetId) ?? entry.assetId);
        entry.decimals = Number((isArray ? result[3] : result?.decimals) ?? entry.decimals);
        entry.collateralRisk = Number((isArray ? result[4] : result?.collateralRisk) ?? entry.collateralRisk);
        entry.dynamicConfigKeyMax = Number((isArray ? result[6] : result?.dynamicConfigKey) ??
    ...
    for (const chain of Object.keys(spokesData)) {
        ...
        for (let i = 0; i < callMeta.length; i++) {
            ...
            if (meta.callType === 'reserve') {
                ...
                const isArray = Array.isArray(result);
                entry.underlying = ((isArray ? result[0] : result?.underlying) ?? '')
                    .toString()
                    .toLowerCase();
                entry.hub = ((isArray ? result[1] : result?.hub) ?? '')
                    .toString()
                    .toLowerCase();
                entry.assetId = Number((isArray ? result[2] : result?.assetId) ?? 0);
                entry.decimals = Number((isArray ? result[3] : result?.decimals) ?? 18);
                entry.collateralRisk = Number((isArray ? result[4] : result?.collateralRisk) ?? 0);
                entry.dynamicConfigKeyMax = Number((isArray ? result[6] : result?.dynamicConfigKey) ?? 0);
            }
            else if (meta.callType === 'config') {
                const entry = spokeReserves[meta.spokeAddr].find((r) => r.reserveId === meta.reserveId);
                if (entry) {
        ...
        for (let retryRound = 0; retryRound < MAX_GET_RESERVE_RETRY_ROUNDS; retryRound++) {
            const retryCalls = [];
            const retryMeta = [];
            for (const [spokeAddr, details] of Object.entries(spokeReserves)) {
                for (const entry of details) {
                    if (!needsGetReserveRetry(entry))
                        continue;
                    retryCalls.push({
                        address: spokeAddr,
                        name: V4FetchFunctions.getReserve,
            ...
            if (retryCalls.length === 0)
                break;
            console.log(`  [${chain}] getReserve retry round ${retryRound + 1}/${MAX_GET_RESERVE_RETRY_ROUNDS}: ${retryCalls.length} slot(s)`);
            let retryResults;
            try {
                ...
                for (let i = 0; i < dynMeta.length; i++) {
                    ...
                    if (entry && result) {
                        ...
                        entry.latestDynamicConfig = {
                            collateralFactor: Number((isArr ? result[0] : result?.collateralFactor) ?? 0),
                            maxLiquidationBonus: Number((isArr ? result[1] : result?.maxLiquidationBonus) ?? 0),
                            liquidationFee: Number((isArr ? result[2] : result?.liquidationFee) ?? 0),
                        };
                    }
                }
            }
        ...
        for (const [spokeAddr, details] of Object.entries(spokeReserves)) {
            ...
            maxDynKeys[chain][spokeAddr] = details.reduce((max, d) => Math.max(max, d.dynamicConfigKeyMax), 0);
        }
        const totalReserves = Object.values(reservesOutput[chain]).reduce((sum, arr) => sum + arr.length, 0);
        console.log(`  Found ${totalReserves} total reserves across ${Object.keys(reservesOutput[chain]).length} spokes`);
    }
