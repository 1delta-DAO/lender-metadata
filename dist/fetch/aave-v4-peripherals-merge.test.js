...
describe("mergePositionManagerLists", () => {
    ...
    it("never lets the API's \"Unknown\" displace a curated PM name", () => {
        ...
        const out = mergePositionManagerLists([{ name: "Aave Taker Position Manager", address: addr, active: true }], [{ name: "Unknown", address: addr, active: true }]);
        expect(out[0].name).toBe("Aave Taker Position Manager");
    });
    it("still takes a real incoming name", () => {
        const addr = "0x6c044c0d3801499bcabfad458b70880bc518e9f7";
        const out = mergePositionManagerLists([{ name: "Unknown", address: addr, active: true }], [{ name: "Aave Native Gateway", address: addr, active: true }]);
...
describe("mergeAaveV4PeripheralsData", () => {
    ...
    it("merges the shape the committed config actually has", () => {
        ...
        expect(out["1"].perHub?.[CORE_HUB]?.signatureGateway).toBe("0xfbc184337dc6595d8bf62968bda46e7de7af9c3d");
        expect(out["1"].perSpoke?.["0x3131fe68c4722e726fe6b2819ed68e514395b9a4"]?.spokeName).toBe("Kelp");
    });
    ...
    it("omits a gateway entirely rather than publishing an empty string", () => {
        ...
        const out = mergeAaveV4PeripheralsData({}, { "10": { nativeGateway: "", signatureGateway: "", perHub: {}, perSpoke: {} } });
    ...
    it("merges position managers per spoke without dropping prior addresses", () => {
        ...
        const pm = (address, name) => ({ name, address, active: true });
        const out = mergeAaveV4PeripheralsData({
            "1": {
                perSpoke: {
                    [addr]: {
    ...
    it("keeps a chain the incoming fetch does not cover at all", () => {
        ...
        const out = mergeAaveV4PeripheralsData({ "10": { perSpoke: { "0xdffcc3536d932eb51df51a7f5fa407c4270d5308": {
        ...
        expect(out["10"].perSpoke?.["0xdffcc3536d932eb51df51a7f5fa407c4270d5308"]?.spokeName).toBe("ether.fi Cash");
    });
});
