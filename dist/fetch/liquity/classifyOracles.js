 ...
 */
export async function classifyLiquityOracles() {
    const markets = readJsonFile(marketsFile);
    ...
    for (const [lender, chains] of Object.entries(markets ?? {})) {
        if (lender.startsWith("_"))
            continue;
        for (const [chainId, branches] of Object.entries(chains ?? {})) {
            for (const br of branches ?? []) {
                if (br?.collIndex == null || !isAddr(br.priceFeed) || !isAddr(br.collToken))
                    continue;
    ...
    for (const [chainId, items] of Object.entries(byChain)) {
        ...
        const anchorRes = (await multicallRetryUniversal({
            ...
            abi: ETH_USD_ORACLE_ABI,
            allowFailure: true,
            maxRetries: 4,
        }).catch(() => []));
        const anchorByFeed = new Map();
