...
const TOKEN_LIST_URL = (chainId) => `https://raw.githubusercontent.com/1delta-DAO/token-lists/main/${chainId}.json`;
const SOURCES = {
...
function add(map, chainId, token) {
    if (!isAddress(token))
        return;
...
export function collectLendingTokens() {
    ...
    if (aaveTokens) {
        for (const byChain of Object.values(aaveTokens)) {
            for (const [chainId, byUnderlying] of Object.entries(byChain ?? {})) {
                for (const t of Object.keys(byUnderlying ?? {}))
                    add(tokens, chainId, t);
            }
        }
    }
    ...
    if (aaveV4Spokes) {
        for (const [chainId, bySpoke] of Object.entries(aaveV4Spokes)) {
            for (const spoke of Object.values(bySpoke ?? {})) {
                for (const r of spoke?.reserves ?? [])
                    add(tokens, chainId, r?.underlying);
            }
        }
    }
    ...
    if (compV2) {
        for (const byChain of Object.values(compV2)) {
            for (const [chainId, list] of Object.entries(byChain ?? {})) {
                for (const e of list ?? [])
                    add(tokens, chainId, e?.underlying);
            }
        }
    }
    ...
    if (compV3Base) {
        for (const byChain of Object.values(compV3Base)) {
    ...
    if (compV3Res) {
        for (const byChain of Object.values(compV3Res)) {
            for (const [chainId, list] of Object.entries(byChain ?? {})) {
                if (Array.isArray(list))
                    for (const t of list)
                        add(tokens, chainId, t);
    ...
    if (euler) {
        for (const byChain of Object.values(euler)) {
            for (const [chainId, list] of Object.entries(byChain ?? {})) {
                for (const e of list ?? [])
                    add(tokens, chainId, e?.underlying);
            }
    ...
    if (fluid) {
        for (const [chainId, byVault] of Object.entries(fluid)) {
            for (const v of Object.values(byVault ?? {})) {
                for (const side of ["borrow", "supply"]) {
                    for (const a of v?.[side]?.assets ?? [])
                        add(tokens, chainId, a?.underlying);
                }
            }
        }
    }
    ...
    if (init) {
        for (const byChain of Object.values(init)) {
            for (const [chainId, list] of Object.entries(byChain ?? {})) {
                for (const e of list ?? [])
                    add(tokens, chainId, e?.underlying);
            }
    ...
    if (morphoOracles) {
        for (const [chainId, byMarket] of Object.entries(morphoOracles)) {
    ...
    if (morphoVaults) {
        for (const byChain of Object.values(morphoVaults)) {
            for (const [chainId, list] of Object.entries(byChain ?? {})) {
                for (const e of list ?? [])
                    add(tokens, chainId, e?.underlying);
            }
    ...
    for (const path of [SOURCES.siloV2, SOURCES.siloV3]) {
        ...
        if (!silo)
            continue;
        for (const [chainId, list] of Object.entries(silo)) {
            for (const m of list ?? []) {
    ...
    return Object.fromEntries(Array.from(tokens.entries())
...
export async function collectMissingLendingTokens(opts = {}) {
    const { includeChainsWithoutList = true } = opts;
    const all = collectLendingTokens();
    const chainIds = Object.keys(all);
    const lists = await Promise.all(chainIds.map(async (chainId) => {
        try {
            return [chainId, await loadTokenList(chainId)];
        }
        catch (err) {
    ...
    for (const [chainId, list] of lists) {
        ...
        if (list === undefined)
            continue; // fetch error — skip rather than mislabel
        if (list === null) {
            if (includeChainsWithoutList)
                missing[chainId] = tokens;
            continue;
        }
