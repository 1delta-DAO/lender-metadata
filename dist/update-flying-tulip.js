...
import { fetchFlyingTulipRoster, buildFlyingTulipLabels, } from "./fetch/flying-tulip/flyingTulip.js";
...
async function main() {
    ...
    const rosterRes = await writeTextIfChanged(ROSTER_FILE, JSON.stringify({ FLYING_TULIP: roster }, null, 2) + "\n");
    ...
    const labelRes = await writeTextIfChanged(LABELS_FILE, JSON.stringify(labels, null, 2) + "\n");
    console.log(`Flying Tulip: ${total} asset(s) across ${chains.length} chain(s) (${rosterRes}); labels (${labelRes})`);
    for (const c of chains) {
        const ch = roster[c];
        console.log(`  chain ${c}: hfSafe=${ch.marginHfSafeBps} hfTarget=${ch.marginHfTargetBps} minEquityUSDWad=${ch.marginMinEquityUSDWad}`);
        for (const a of ch.assets) {
            const flags = [
                ...
                .filter(Boolean)
                .join(", ");
            console.log(`    ${a.symbol.padEnd(8)} mm=${String(a.mmBps).padStart(4)}  ${flags}`);
        }
    }
    process.exit(0);
}
