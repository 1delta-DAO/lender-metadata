...
function lltvDecimalToBps(lltv) {
    if (!Number.isFinite(lltv))
        return "0";
    return Math.round(lltv * 100).toString();
}
function enumName(marketId) {
...
async function main() {
    ...
    if (!labels.names)
        labels.names = {};
    if (!labels.shortNames)
        labels.shortNames = {};
    let labelsAdded = 0;
    const unlabeledByChain = {};
    for (const [chainId, infos] of Object.entries(markets)) {
        for (const info of infos) {
            ...
            if (wasUnlabeled) {
                if (!unlabeledByChain[chainId])
                    unlabeledByChain[chainId] = [];
    ...
    if (labelsAdded > 0) {
        ...
        const labelsResult = await writeTextIfChanged(LABELS_FILE, JSON.stringify(labels, null, 2) + "\n");
        console.log(`Added/updated ${labelsAdded} Mystic labels; file ${labelsResult}.`);
    }
    else {
    ...
    // ---- missing-asset check ---------------------------------------------
    const missing = [];
    const assetCheckTargets = Object.keys(unlabeledByChain).length > 0 ? unlabeledByChain : newIdsByChain;
