 ...
 */
function protocolFor(lenderKey) {
    const key = lenderKey.toUpperCase();
    let best;
    for (const family of Object.keys(PROTOCOL_BY_FAMILY)) {
        if (key !== family && !key.startsWith(`${family}_`))
            continue;
        if (!best || family.length > best.length)
            best = family;
    }
    return best ? PROTOCOL_BY_FAMILY[best] : undefined;
}
async function main() {
    const labels = (await readJsonFile(LABELS_FILE));
    ...
    const changed = await writeTextIfChanged(LABELS_FILE, `${JSON.stringify(next, null, 2)}\n`);
    const families = new Set(Object.values(protocols));
    console.log(`protocol labels: ${Object.keys(protocols).length} keys -> ` +
