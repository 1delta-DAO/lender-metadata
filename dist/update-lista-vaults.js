...
async function main() {
    ...
    console.log(`Fetched ${totalFetched} Lista vaults across ${Object.keys(vaults).length} chains`);
    let existing = {};
    try {
    ...
    for (const [chainId, infos] of Object.entries(vaults)) {
        ...
        const nameByAddr = new Map(infos.map((i) => [i.address.toLowerCase(), i.name]));
