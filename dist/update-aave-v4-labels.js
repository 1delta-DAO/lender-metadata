...
import { buildAaveV4Labels, discoverAaveV4SpokeNames, fetchSpokePositionManagers, isPlaceholderSpokeLabel, } from "./fetch/aave/v4SpokeLabels.js";
...
async function main() {
    ...
    if (discovered.length === 0) {
        ...
        // spoke on every hub.
        console.error("Aave V4: the GraphQL API returned no spokes on any seeded hub — refusing to write labels.");
        process.exit(1);
    }
    // ---------------------------------------------------------------------
    // 1. config/aave-v4-peripherals.json — curated spoke names
    // ---------------------------------------------------------------------
    const peripherals = readJsonFile(PERIPHERALS_FILE);
    const renamed = [];
    const added = [];
    for (const s of discovered) {
        ...
        if (!existing) {
            ...
            let positionManagers = [];
            try {
                positionManagers = await fetchSpokePositionManagers(s.spokeId);
            }
            catch (e) {
                console.warn(`[aave-v4-labels] chain ${s.chainId} spoke ${s.spoke}: position-manager query failed ` +
        ...
        if (s.spokeId && existing.spokeId !== s.spokeId)
            existing.spokeId = s.spokeId;
        if (existing.spokeName !== s.name) {
            renamed.push(`${s.chainId}:${s.spoke} "${existing.spokeName ?? ""}" -> "${s.name}"`);
            existing.spokeName = s.name;
        }
    }
    const periphRes = await writeTextIfChanged(PERIPHERALS_FILE, JSON.stringify(peripherals, null, 2) + "\n");
    // ---------------------------------------------------------------------
    ...
    const spokesJson = readJsonFile(SPOKES_FILE);
    const fromSpokesFile = [];
    ...
    const labelsRes = await writeTextIfChanged(LABELS_FILE, JSON.stringify(labels, null, 2) + "\n");
    // ---------------------------------------------------------------------
    // 3. Report
    // ---------------------------------------------------------------------
    console.log(`Aave V4 labels: ${Object.keys(built.names).length} spoke label(s), ` +
        `${newKeys.length} new (${labelsRes})`);
    for (const k of newKeys)
        console.log(`  + ${k} = ${built.names[k]}`);
