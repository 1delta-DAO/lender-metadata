...
async function fetchCuratorRoster() {
    ...
    if (!res.ok)
        throw new Error(`Morpho curators query failed: ${res.status}`);
    const json = await res.json();
    if (json?.errors?.length) {
        throw new Error(`Morpho curators GraphQL error: ${json.errors.map((e) => e?.message).join("; ")}`);
    }
    const items = json?.data?.curators?.items ?? [];
    ...
    for (const [addr, names] of byAddress) {
        if (names.size === 1)
            out.set(addr, [...names][0]);
        else
            ambiguous++;
    }
    console.log(`Curator roster: ${items.length} curators, ${out.size} usable addresses (${ambiguous} ambiguous dropped)`);
    return out;
}
// ---------------------------------------------------------------------------
...
function curatorFromVaultName(name, assetSymbol) {
    if (!name)
        return undefined;
    // A test/fake vault's leading word is not a curator, whatever the casing.
    if (/\b(test|testing|fake)\b/i.test(name))
        return undefined;
    ...
    if (!terminated || out.length === tokens.length)
        return undefined;
...
async function readGovernance(chainId, vaults) {
    const out = new Map();
    if (vaults.length === 0)
        return out;
...
// ---------------------------------------------------------------------------
async function main() {
    const existing = readJsonFile(VAULTS_FILE);
    ...
    let unresolved = 0;
    const failures = [];
    const chainIds = Object.keys(byChain).filter((c) => CHAIN_FILTER.size === 0 || CHAIN_FILTER.has(c));
    await Promise.all(chainIds.map(async (chainId) => {
        const entries = byChain[chainId] ?? [];
        if (entries.length === 0)
            return;
        let governance = new Map();
        let symbols = new Map();
        try {
            const underlyings = [
            ...
            [governance, symbols] = await Promise.all([
                readGovernance(chainId, entries.map((e) => e.vault.toLowerCase())),
            ...
            console.warn(`  chain ${chainId}: on-chain reads failed: ${err?.message ?? err}`);
        }
        for (const entry of entries) {
            const vault = entry.vault.toLowerCase();
            const gov = governance.get(vault) ?? {};
            const manual = MANUAL_CURATORS[chainId]?.[vault]?.trim();
            const rosterName = (gov.curator && roster.get(gov.curator)) ||
                (gov.owner && roster.get(gov.owner)) ||
                undefined;
            if (manual) {
                if (entry.curatorName !== manual)
                    entry.curatorName = manual;
                fromManual++;
                continue;
            }
            if (rosterName) {
                // The roster is authoritative — it may refresh a stale stored name.
                if (entry.curatorName !== rosterName)
                    entry.curatorName = rosterName;
                fromRoster++;
                continue;
            }
            if (entry.curatorName)
                continue; // keep what a human/roster once set
            const derived = curatorFromVaultName(entry.name, symbols.get(entry.underlying.toLowerCase()));
            if (derived) {
    ...
    console.log(`Curators: ${fromManual} manual, ${fromRoster} roster, ${fromName} name-derived, ${unresolved} unresolved; file ${writeResult}.`);
    if (failures.length > 0) {
        console.warn(`Governance reads failed on ${failures.length} chain(s): ${failures.join(", ")}`);
    }
    process.exit(0);
}
main().catch((err) => {
