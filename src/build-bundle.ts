...
interface Manifest {
  version: number;
  /** Paths allowed to be absent — a lender whose config is not published yet. */
  optional?: string[];
  files: string[];
}
...
function main(): void {
  ...
  for (const relPath of [...manifest.files].sort()) {
    ...
    if (!existsSync(abs)) {
      ...
      if (!optional.has(relPath)) unexpectedlyMissing.push(relPath);
      continue;
    }
    try {
  ...
  if (unexpectedlyMissing.length > 0) {
    console.error(
      `[build-bundle] ${unexpectedlyMissing.length} manifest path(s) do not exist and are not marked optional:`,
    );
    for (const p of unexpectedlyMissing) console.error(`  - ${p}`);
    console.error(
      "Add the file, or add it to `optional` in bundle/manifest.json if it is genuinely unpublished.",
    );
    process.exit(1);
  }
  ...
  console.log(
    `[build-bundle] wrote bundle/lender-meta.json — ${Object.keys(files).length} files, ${(bytes / 1e6).toFixed(2)} MB`,
  );
  if (missing.length > 0) {
    console.log(
      `[build-bundle] skipped ${missing.length} optional path(s): ${missing.join(", ")}`,
    );
  }
}
