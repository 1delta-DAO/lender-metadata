...
const VAULT_TYPE = { EULER_V2: 0, AAVE_V3: 1 } as const;

const DISPLAY = { TWYNE: { name: "Twyne", short: "Twyne" } } as const;

type TwyneChainCfg = {
  evc: string;
  collateralVaultFactory: string;
  vaultManager: string;
  oracleRouter?: string;
  aavePool?: string;
  [k: string]: any;
};
type TwyneConfig = Record<string, Record<string, TwyneChainCfg>>;
...
function readConfig(): TwyneConfig {
  try {
...
const marketKey = (chainId: string, iv: string, targetAsset: string) =>
  `TWYNE_${chainId}_${iv.replace(/^0x/i, "").toUpperCase()}_${targetAsset
    .replace(/^0x/i, "")
    .toUpperCase()}`;
 ...
 */
async function scanTriples(chainId: string, vaultManager: string) {
  let logs: any[] = [];
  let lastErr: any;
  ...
  const pinned =
    process.env[`TWYNE_RPC_URL_${chainId}`] ?? process.env.TWYNE_RPC_URL;
  const clients: any[] = [];
  if (pinned) {
  ...
  for (const client of clients) {
    try {
      logs = await client.getLogs({
        address: vaultManager as any,
        events: VAULT_MANAGER_EVENTS as any,
        fromBlock: 0n,
        toBlock: "latest",
      });
  ...
  if (!logs.length) {
    throw new Error(
      `Twyne: VaultManager whitelist scan returned nothing across ${clients.length} endpoints (${
        (lastErr as any)?.shortMessage ?? (lastErr as any)?.message ?? "empty result"
      }). Refusing to publish an empty roster.`,
    );
  }

  const activeIvs = new Set<string>();
  const targetVaults = new Map<string, Set<string>>(); // iv -> targetVaults
  const triples = new Map<string, { iv: string; tv: string; asset: string }>();

  for (const log of logs) {
    const a = log.args ?? {};
    if (log.eventName === "T_SetIntermediateVault") {
      const iv = lower(a.intermediateVault);
      if (a.value) activeIvs.add(iv);
      else activeIvs.delete(iv);
    } else if (log.eventName === "T_AddAllowedTargetVault") {
      const iv = lower(a.intermediateVault);
      if (!targetVaults.has(iv)) targetVaults.set(iv, new Set());
  ...
  const eulerPairs: { iv: string; tv: string }[] = [];
  for (const [iv, tvs] of targetVaults) {
    for (const tv of tvs) {
      const alreadyTripled = [...triples.values()].some(
        (t) => t.iv === iv && t.tv === tv,
      );
      if (!alreadyTripled) eulerPairs.push({ iv, tv });
    }
  }
...
async function fetchChain(chainId: string, cfg: TwyneChainCfg) {
  const { activeIvs, triples, eulerPairs } = await scanTriples(
    chainId,
    cfg.vaultManager,
  );
  ...
  const candidates: Candidate[] = [
    ...
    ...eulerPairs.flatMap((p, i) => {
      ...
      return asset
        ? [{ iv: p.iv, tv: p.tv, asset, vaultType: "EULER_V2" as const }]
        : [];
    }),
  ...
  const ivInfo = new Map<string, any>();
  ivs.forEach((iv, i) => {
    const o = i * 6;
    ivInfo.set(iv, {
  ...
  const collAssets = [
    ...new Set(
      [...ivInfo.values()].map((v) => v.collateralAsset).filter(Boolean),
    ),
  ] as string[];
  ...
  const collInfo = new Map<string, any>();
  ...
  const underInfo = new Map<string, any>();
  ...
  candidates.forEach((c, i) => {
    ...
    if (!allowed || !iv?.registered || !iv.collateralAsset) return;
  ...
  for (const m of markets) {
    ...
    if (seen && seen !== triple) {
      throw new Error(
        `Twyne: key ${m.key} maps to two triples (${seen} / ${triple}). ` +
...
export class TwyneUpdater implements DataUpdater {
  name = "twyne";
  defaults = {};
  ...
  async fetchData(): Promise<{ [file: string]: any }> {
    ...
    for (const [lender, chains] of Object.entries(config)) {
      const disp = DISPLAY[lender as keyof typeof DISPLAY] ?? DISPLAY.TWYNE;
      for (const [chainId, cfg] of Object.entries(chains)) {
        if (!cfg?.vaultManager || !cfg?.collateralVaultFactory) continue;
        try {
          ...
          const fixedTerm = data.markets.filter(
            (m: any) => m.collateralMaturity,
          ).length;
          console.log(
            `Twyne: chain ${chainId}: ${data.markets.length} markets ` +
              `(${fixedTerm} fixed-term collateral), ` +
              `${new Set(data.markets.map((m: any) => m.intermediateVault)).size} credit vaults`,
          );
        } catch (e) {
          // Never publish a partial roster: a failed scan must leave the
          // previous, known-good rows in place (mergeData keeps them).
          console.log(
            `Twyne: chain ${chainId} failed —`,
            (e as any)?.message ?? e,
          );
        }
      }
  ...
  mergeData(oldData: any, data: any, fileKey?: string): any {
    ...
    for (const [lender, chains] of Object.entries(
      ...
      for (const [chainId, chainData] of Object.entries(chains)) {
        if ((chainData as any)?.markets?.length) {
