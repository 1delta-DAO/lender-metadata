 ...
 */
const COLLATERAL_ALLOWLIST: Record<string, Record<string, string>> = {
  "1": {
...
type FrankencoinChainCfg = {
  zchf: string;
  mintingHub: string;
  [k: string]: any;
};
type FrankencoinConfig = Record<string, Record<string, FrankencoinChainCfg>>;
...
function readConfig(): FrankencoinConfig {
...
async function fetchCandidates(chainId: string): Promise<any[]> {
  const data = await fetchJson(`${API_BASE}/positions/open`);
  const map: Record<string, any> = data?.map ?? {};
  const allow = COLLATERAL_ALLOWLIST[chainId] ?? {};
  const out: any[] = [];
  let skippedV1 = 0;
  let skippedColl = new Set<string>();
  for (const p of Object.values(map)) {
    if (!p?.isOriginal) continue; // clones are user positions, not markets
  ...
  if (skippedV1 > 0) console.log(`Frankencoin: skipped ${skippedV1} non-V2 positions`);
  if (skippedColl.size > 0) {
    console.log(
      `Frankencoin: skipped unpriceable collateral (not in allowlist): ${[...skippedColl].join(", ")}`,
    );
  }
  return out;
}
...
async function fetchChain(
  ...
  for (const cand of candidates) {
    ...
    try {
      ...
      console.log(
        `Frankencoin: ${position} verification failed:`,
...
export class FrankencoinUpdater implements DataUpdater {
  ...
  async fetchData(): Promise<{ [file: string]: any }> {
    ...
    for (const lender of lenders) {
      const disp = DISPLAY[lender as keyof typeof DISPLAY] ?? { name: lender, short: lender };
      ...
      for (const [chainId, cfg] of Object.entries(config[lender])) {
        try {
          ...
          console.log(
            `Frankencoin: ${lender} chain ${chainId} failed:`,
            (e as any)?.shortMessage ?? (e as any)?.message ?? e,
          );
        }
      }
