...
export type TellerOracleRoute = {
  /** The Uniswap-family pool acting as the price oracle. */
  pool: string;
  zeroForOne: boolean;
  /** TWAP window in seconds; 0 = spot slot0() (manipulable). */
  twapInterval: number;
};

export type TellerOraclePoolData = {
  /** LenderCommitmentGroup pool address — the leaf id (marketUid = TELLER_<pool>). */
  market: string;
  principal: string;
  collateral: string;
  principalSymbol: string | null;
  collateralSymbol: string | null;
  /** Pool generation: 2 = Uniswap-V3 TWAP routes, 3 = pluggable price adapter. */
  generation: 2 | 3 | null;
  /** DEX oracle family. */
  provider: "uniswap-v3-twap" | "teller-price-adapter" | "unknown";
  /** Primary oracle contract: the Uniswap pool (V2) or the adapter (V3). */
  oracle: string | null;
  /** V2 only: the decoded route(s). */
  routes: TellerOracleRoute[];
  /** V3 only: adapter + route hash (underlying pool lives inside the adapter). */
  priceAdapter: string | null;
  priceRouteHash: string | null;
  /** Shortest TWAP window across routes (0 = a spot route exists). */
  minTwapInterval: number | null;
  /**
   * Teller oracles quote principal-PER-COLLATERAL, i.e. in the COLLATERAL token,
   * NOT USD. USD conversion needs an external price for one leg (handled by the
   * margin-fetcher teller oracle fetcher, which anchors to whichever side has a
   * known USD price).
   */
  denominator: string | null;
  intendedPair: string | null;
  /**
   * `correctOracle`: true when the pool prices via a real TWAP (every route has
   * `twapInterval > 0`); FALSE when any route uses spot `slot0()` (manipulable);
   * null when the oracle couldn't be read. This is the Teller-specific risk
   * signal — a thin-liquidity DEX pool + short/zero TWAP is manipulable.
   */
  correctOracle: boolean | null;
};

export type TellerOraclesClassifiedMap = {
  [chainId: string]: { [pool: string]: TellerOraclePoolData };
};
 ...
 */
export async function classifyTellerOracles(): Promise<TellerOraclesClassifiedMap> {
  const poolsByChain = readJsonFile(poolsFile) as Record<
    string,
    Array<{
      pool: string;
      principal: string;
  ...
  const result: TellerOraclesClassifiedMap = {};
  ...
  for (const [chainId, pools] of Object.entries(poolsByChain ?? {})) {
    ...
    const res = (await multicallRetryUniversal({
      chain: chainId,
      calls,
      abi: POOL_ORACLE_ABI as any,
      allowFailure: true,
      maxRetries: 4,
    }).catch(() => [])) as any[];

    const route = (r: any): TellerOracleRoute | null => {
      if (!r || r === "0x") return null;
      const pool = Array.isArray(r) ? toAddr(r[0]) : toAddr(r?.pool);
      if (!isAddr(pool)) return null;
    ...
    rows.forEach((p, i) => {
      ...
      const routeHash =
        typeof res[b + 3] === "string" && res[b + 3] !== "0x" ? res[b + 3] : null;

      const routes = [r0, r1].filter((x): x is TellerOracleRoute => !!x);
