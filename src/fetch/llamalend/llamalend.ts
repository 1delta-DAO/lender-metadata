...
type LlamaLendConfig = Record<string, Record<string, LlamaLendChainCfg>>;
...
export class LlamaLendUpdater implements DataUpdater {
  name = "llamalend";
  defaults = {};
  ...
  async fetchData(): Promise<{ [file: string]: any }> {
    ...
    try {
      ...
      console.log(
        "LlamaLend: Curve API unreachable —",
        (e as any)?.message ?? e,
      );
      return { [MARKETS_FILE]: {}, [LABELS_FILE]: { names, shortNames } };
    }
    const rows: any[] = api?.data?.lendingVaultData ?? [];
    if (rows.length === 0) {
    ...
    for (const [lender, chains] of Object.entries(config)) {
      const disp = DISPLAY[lender as keyof typeof DISPLAY] ?? DISPLAY.LLAMALEND;
      for (const [chainId, cfg] of Object.entries(chains)) {
        if (!cfg.oneWayFactory && !cfg.lendFactory) continue;
        try {
          ...
          console.log(
            `LlamaLend: ${lender} chain ${chainId} failed:`,
...
function readConfig(): LlamaLendConfig {
  try {
...
function classifyCode(
  ...
  const has = (sel: string) => code.includes(sel);
  ...
  if (
    ...
    console.log(
      `LlamaLend: chain ${chainId}: ${controller} has approve() but not the full delegation set — treating as NOT delegatable (blueprint shape changed?)`,
    );
    return false;
  }
  return hasApprove;
...
function rateModelFor(
  minRate: any,
  rateCalculator: any,
  version: 1 | 2,
): "semilog" | "hyperbolic" | "hyperbolic-dynamic" {
...
async function fetchChain(
  ...
  const candidates = apiRows.filter((r: any) => {
    if (r?.blockchainId !== slug) return false;
    if (!r?.controllerAddress || !r?.address || !r?.ammAddress) return false;
  ...
  const calls = candidates.flatMap((r: any) =>
    MARKET_READS.map(({ on, name }) => ({
      address:
        on === "controller"
          ? r.controllerAddress
          : on === "amm"
  ...
  const delegation = await scanDelegation(
    chainId,
    candidates.map((r: any) => r.controllerAddress),
  );
  ...
  if (unresolved.length > 0) {
    throw new Error(
      `delegation scan unresolved for ${unresolved.length}/${candidates.length} markets — refusing to write a roster with an unverified supportsDelegation column`,
    );
  }

  const markets: any[] = [];
  candidates.forEach((r: any, i: number) => {
    ...
    const [
      ...
      minRate,
      rateCalculator,
    ] = MARKET_READS.map((_, j) => verify[base + j]);
    ...
    if (
      ...
      console.log(
        `LlamaLend: chain ${chainId}: DROPPING ${r.name ?? r.controllerAddress} — on-chain verification failed`,
      );
      return;
    }
    if (typeof ammA !== "bigint" || ammA <= 1n) {
      console.log(
        `LlamaLend: chain ${chainId}: DROPPING ${r.controllerAddress} — unreadable AMM A`,
      );
      return;
    }
    ...
    markets.push({
      ...
      ammA: ammA.toString(),
      loanDiscount:
        typeof loanDiscount === "bigint" ? loanDiscount.toString() : "0",
      liquidationDiscount:
        typeof liquidationDiscount === "bigint"
      ...
      borrowCap:
        version === 2 && typeof borrowCap === "bigint"
          ? borrowCap.toString()
          : undefined,
