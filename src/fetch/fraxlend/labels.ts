...
export interface FraxlendPairLabel {
  chainId: string;
...
export function fraxlendLenderKey(chainId: string, pair: string): string {
 ...
 */
export async function discoverFraxlendPairs(): Promise<FraxlendPairLabel[]> {
  ...
  const out: FraxlendPairLabel[] = [];

  for (const [chainId, chainData] of Object.entries<any>(byChain)) {
    const listed = (Array.isArray(chainData?.pairs) ? chainData.pairs : []).filter(
      (p: any) => isAddr(p?.address),
    );
    ...
    legs.forEach((r) => {
      if (r.status === "success" && isAddr(r.result))
        tokens.add((r.result as string).toLowerCase());
    });
    ...
    listed.forEach((p: any, i: number) => {
      ...
      let assetSymbol =
        asset?.status === "success" && isAddr(asset.result)
          ? symbolOf.get((asset.result as string).toLowerCase())
          : undefined;
      let collateralSymbol =
        coll?.status === "success" && isAddr(coll.result)
          ? symbolOf.get((coll.result as string).toLowerCase())
          : undefined;

      // Fall back to the curated label, which is `<asset> / <collateral>`.
      if ((!assetSymbol || !collateralSymbol) && typeof p.label === "string") {
        const [a, c] = p.label.split("/").map((s: string) => s.trim());
        assetSymbol ||= a;
        collateralSymbol ||= c;
      }
      if (!assetSymbol || !collateralSymbol) {
        console.warn(
          `  ~ ${p.address}: could not resolve both legs — skipped`,
        );
        return;
      }
      out.push({
...
export function buildFraxlendLabels(pairs: FraxlendPairLabel[]): {
  names: Record<string, string>;
  shortNames: Record<string, string>;
} {
  const names: Record<string, string> = { FRAXLEND: "Fraxlend" };
  const shortNames: Record<string, string> = { FRAXLEND: "Fraxlend" };
