...
import { erc20Abi, hexToString, stringToHex } from "viem";
...
type SkyConfig = Record<string, Record<string, SkyChainCfg>>;
 ...
 */
const at = (res: any, idx: number): any => {
  const v = res?.[idx];
  return typeof v === "function" ? undefined : v;
};

const ilkToString = (b: string): string => {
  try {
    return hexToString(b as `0x${string}`, { size: 32 }).replace(/[\s\0]+$/g, "");
  } catch {
    return "";
  }
};
...
async function fetchChain(
  ...
  const keep: { ilk32: string; ilk: string; info: any }[] = [];
  ...
  for (let i = 0; i < keep.length; i++) {
    ...
    const ilkDebt =
      (BigInt(vat.Art ?? vat[0]) * BigInt(vat.rate ?? vat[1])) / 10n ** 45n;
...
export class SkyUpdater implements DataUpdater {
    ...
    for (const lender of lenders) {
      ...
      for (const [chainId, cfg] of Object.entries(config[lender])) {
        try {
          ...
          const live = data.markets.filter((m: any) => !m.offboarded).length;
          console.log(
          ...
          console.log(
            `Sky: ${lender} chain ${chainId} failed:`,
