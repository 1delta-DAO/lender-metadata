...
} from "./v4SpokeLabels.js";

describe("aaveV4LenderKey", () => {
  it("matches the lender id shape (`aave-v4-<spoke>` uppercased)", () => {
    expect(aaveV4LenderKey("0x774b9655413c34809c1f1b16b654465A89EBE989")).toBe(
      "AAVE_V4_774B9655413C34809C1F1B16B654465A89EBE989",
    );
  });
...
describe("discoverAaveV4SpokeNames", () => {
  ...
  function gqlStub(bodies: Record<string, { id: string; address: string; name: string }[]>) {
    return (async (_url: string, init: any) => {
      const hub = JSON.parse(init.body).variables.hub;
      const spokes = bodies[hub];
      if (!spokes) throw new Error(`boom for ${hub}`);
