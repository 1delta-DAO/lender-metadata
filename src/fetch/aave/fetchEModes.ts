 ...
 */
function isActive(result: any): boolean {
  if (!result || typeof result !== "object") return false;
  return (
    Number(result.ltv ?? 0) !== 0 ||
...
type Probe = {
  ...
  highest: number;
  clean: boolean;
  /** Pool answered nothing while other pools on the chain answered fine. */
  unsupported: boolean;
};

export type EModeCounts = {
  [fork: string]: { [chainId: string]: { eModeCount: number } };
};
 ...
 */
export async function fetchAaveEModeCounts(
  AAVE_FORK_POOL_DATA: any,
): Promise<EModeCounts> {
  const counts: EModeCounts = {};

  // Group deployments by chain so one multicall covers every fork on it.
  const chainToForks: { [chain: string]: { fork: string; pool: string }[] } = {};
  for (const fork of Object.keys(AAVE_FORK_POOL_DATA)) {
    if (isAaveV2Type(fork) || NO_E_MODES.has(fork)) continue;
  ...
  for (const chain of Object.keys(chainToForks)) {
    ...
    while (pending.length && from <= MAX_E_MODE_ID) {
      ...
      const resolved: any[][] = pending.map(() =>
        new Array(ids.length).fill(undefined),
      );
      let outstanding = pending.flatMap((_, probeIndex) =>
        ids.map((_id, idIndex) => ({ probeIndex, idIndex })),
      );
      ...
      for (let attempt = 0; attempt < ATTEMPTS && outstanding.length; attempt++) {
        ...
        try {
          ...
          console.warn(
            `  e-modes: chain ${chain} ids ${from}-${to} pass ${attempt + 1}/${ATTEMPTS} failed:`,
        ...
        let stillOutstanding: typeof outstanding = [];
        outstanding.forEach((slot, i) => {
          if (isFailure(results[i])) stillOutstanding.push(slot);
          else resolved[slot.probeIndex][slot.idIndex] = results[i];
        });
        ...
        if (anySuccess) {
          ...
          if (dead.size) {
            ...
            stillOutstanding = stillOutstanding.filter(
              (slot) => !dead.has(slot.probeIndex),
            );
          }
        }
        ...
        if (stillOutstanding.length && stillOutstanding.length < outstanding.length) {
          console.log(
            `  e-modes: chain ${chain} ids ${from}-${to} pass ${attempt + 1}/${ATTEMPTS} left ${stillOutstanding.length} call(s) outstanding`,
          );
        }
        outstanding = stillOutstanding;
        if (outstanding.length) await sleep(RETRY_DELAY_MS);
      }

      const next: Probe[] = [];
      pending.forEach((probe, forkIndex) => {
        const slice = resolved[forkIndex];
        if (slice.some((r) => r === undefined)) probe.clean = false;
        slice.forEach((result, i) => {
          if (isActive(result)) probe.highest = Math.max(probe.highest, ids[i]);
        });
    ...
    for (const probe of settled) {
      ...
      if (!probe.clean) {
        console.warn(
          `  e-modes: ${probe.fork} on chain ${chain} had failed reads — keeping the existing count`,
        );
        continue;
      }
      if (!counts[probe.fork]) counts[probe.fork] = {};
      counts[probe.fork][chain] = { eModeCount: probe.highest };
    }

    console.log(
      `  e-modes chain ${chain}: ${settled
        .filter((p) => p.clean)
