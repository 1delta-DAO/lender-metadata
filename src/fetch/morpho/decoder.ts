type Market = {
    ...
    price: bigint // 32 bytes (for morpho)
    loanTokenPrice?: bigint // 32 bytes (for moolah)
    collateralTokenPrice?: bigint // 32 bytes (for moolah)
 ...
 */
export function decodeMorphoMarkets(bytes: Uint8Array): Market[] {
    if (bytes.length % MORPHO_RECORD_SIZE !== 0) {
 ...
 */
export function decodeMoolahMarkets(bytes: Uint8Array): Market[] {
