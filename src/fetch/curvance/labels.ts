 ...
 */
export async function discoverCurvanceMarkets(): Promise<CurvanceMarket[]> {
  const pc = client();
  ...
  for (const mm of managers) {
    ...
    try {
      ...
      console.log(
        `Curvance: ${mm} queryTokensListed failed:`,
    ...
    if (legs.length < 2) {
      console.log(
        `Curvance: ${mm} resolved only ${legs.length} leg(s) — skipped`,
      );
      continue;
    }
 ...
 * on.
 */
export function buildCurvanceLabels(markets: CurvanceMarket[]): BuiltLabels {
  const names: Record<string, string> = { CURVANCE: "Curvance" };
  const shortNames: Record<string, string> = { CURVANCE: "Curvance" };
