...
import {
  discoverFraxlendPairs,
  buildFraxlendLabels,
} from "./fetch/fraxlend/labels.js";
...
async function main() {
  ...
  if (pairs.length === 0) {
    ...
    console.error(
      "Fraxlend: no pairs resolved — refusing to write labels.",
    );
    process.exit(1);
  }
  ...
  console.log(
    `Fraxlend labels: ${Object.keys(built.names).length - 1} pair label(s) + base (${res})`,
  );
