...
import {
  aaveV4PmCanonicalName,
  classifyAaveV4Pm,
} from "./fetch/aave/classifyV4PositionManagers.js";
...
async function makeClient(chainId: string) {
...
function isUnknown(name: unknown): boolean {
  return !name || String(name).trim().toLowerCase() === "unknown";
}

async function main() {
  const data = readJsonFile(FILE) as Record<string, any>;
  ...
  for (const chainId of Object.keys(data)) {
    ...
    if (!client) {
      ...
      console.warn(
        `[aave-v4-pm-names] chain ${chainId}: no reachable RPC (set AAVE_V4_PM_RPC_${chainId}) — ${targets.length} PM(s) left Unknown`,
      );
      continue;
    }

    // De-dup bytecode fetches by address (a PM can serve several spokes).
    const kindByAddr = new Map<string, ReturnType<typeof classifyAaveV4Pm>>();
    for (const addr of new Set(targets.map((t) => t.address))) {
      try {
        ...
        console.warn(
          `[aave-v4-pm-names] chain ${chainId}: getBytecode(${addr}) failed: ${e?.message ?? e}`,
        );
      }
    }
  ...
  console.log(
    `[aave-v4-pm-names] ${result}: renamed ${renamed} PM(s)` +
