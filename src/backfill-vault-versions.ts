...
async function main(): Promise<void> {
  ...
  const data: MorphoTypeVaultsByFork = readJsonFile(VAULTS_FILE);

  // Collect (fork, chain) groups that have at least one vault needing a version.
  const groups: { fork: string; chainId: string }[] = [];
  for (const [fork, byChain] of Object.entries(data)) {
    for (const [chainId, vaults] of Object.entries(byChain)) {
      ...
      if (needs && vaults.length > 0) groups.push({ fork, chainId });
    }
  }
  console.log(
    `Probing ${groups.length} fork/chain groups${refreshAll ? " (refresh all)" : ""}`,
  );
  ...
  await Promise.all(
    groups.map(async ({ fork, chainId }) => {
      ...
      for (let i = 0; i < vaults.length; i++) {
        ...
        if (!version) continue;
        if (!refreshAll && vaults[i].version) continue;
        if (vaults[i].version !== version) {
          if (vaults[i].version) changed++;
          else set++;
  ...
  console.log(
    `Set ${set} new versions, changed ${changed}; file ${writeResult}.`,
  );
  if (failures.length > 0) {
    console.warn(
      `Could not probe ${failures.length} group(s) (unreachable RPC): ${failures.join(", ")}`,
    );
  }
  process.exit(0);
}
