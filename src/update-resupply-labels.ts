...
import {
  discoverResupplyPairs,
  buildResupplyLabels,
} from "./fetch/resupply/labels.js";
...
async function main() {
  ...
  if (pairs.length === 0) {
    ...
    console.error(
      "Resupply: discovery returned no pairs — refusing to write labels.",
    );
    process.exit(1);
  }
  ...
  console.log(
    `Resupply labels: ${Object.keys(built.names).length - 1} pair label(s) + base (${res})`,
  );
  for (const p of pairs) console.log(`  ${p.label}  <-  ${p.rawName}`);
  process.exit(0);
}
