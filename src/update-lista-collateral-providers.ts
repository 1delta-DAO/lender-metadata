...
/**
 * `erc20` — Morpho-shaped: takes the collateral token itself and forwards the
 * Moolah selectors verbatim. The slisBNB provider.
 * `native` — the chain-level provider from `lista-providers.json`: same
 * selectors plus the WBNB/WETH-native variants, wraps for the user.
 * `smart-lp` — SmartProvider: zaps a two-coin StableSwap pool. Different
 * selectors, different inputs, no user-holdable collateral token.
 */
type ProviderKind = "erc20" | "native" | "smart-lp";

type Coin = {
  address: string;
  symbol: string;
  decimals: number;
  /** true when the pool holds the native coin behind the 0xEeee… sentinel */
  isNative: boolean;
};
...
type Roster = Record<string, Record<string, ProviderEntry>>;

function client(chainId: string): PublicClient {
...
async function collectChain(
  ...
  const providers = await Promise.all(
    marketIds.map((id, i) =>
      c.readContract({
        ...
        args: [id as `0x${string}`, (params[i] as any)[1] as Address],
      }),
    ),
  );

  const marketsPerProvider = new Map<string, number>();
  ...
  for (const provider of [...marketsPerProvider.keys()].sort()) {
    ...
    if (!token) {
      throw new Error(
        `chain ${chainId}: provider ${provider} has no TOKEN() — unknown shape, ` +
          `inspect it before publishing (serving ${marketsPerProvider.get(provider)} markets)`,
      );
    }
    const collateralToken = (token as string).toLowerCase();
    ...
    if (!dex) {
      out[provider] = {
        kind:
          nativeProvider && nativeProvider.toLowerCase() === provider
            ? "native"
            : "erc20",
        collateralToken,
    ...
    if ((poolLp as string).toLowerCase() !== (dexLp as string).toLowerCase()) {
      throw new Error(
        `chain ${chainId}: provider ${provider} dexLP ${dexLp} != pool token ${poolLp}`,
      );
    }
    ...
    for (const [i, expected] of [t0, t1].entries()) {
      ...
      if ((coin as string).toLowerCase() !== (expected as string).toLowerCase()) {
        throw new Error(
          `chain ${chainId}: provider ${provider} token(${i}) ${expected} != dex.coins(${i}) ${coin}`,
        );
      }
    }
    ...
    const coins = (await Promise.all(
      [t0, t1].map(async (t) => {
        const address = (t as string).toLowerCase();
    ...
    out[provider] = {
      ...
      collateralSymbol: collateralMeta.symbol,
      collateralDecimals: collateralMeta.decimals,
      dex: (dex as string).toLowerCase(),
      dexInfo: (dexInfo as string).toLowerCase(),
      dexLp: (dexLp as string).toLowerCase(),
      coins,
    };
  }
...
async function main(): Promise<void> {
  ...
  for (const chainId of Object.keys(markets).sort()) {
    const ids: string[] = markets[chainId] ?? [];
    const moolah: string | undefined = pools[chainId];
    if (!moolah) throw new Error(`No Moolah address for chain ${chainId}`);
    if (!ids.length) continue;
    roster[chainId] = await collectChain(
      chainId,
      getAddress(moolah),
      ids,
      natives[chainId]?.nativeProvider,
    );
  }
